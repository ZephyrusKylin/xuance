...
class MultiAgentSatelliteEnv(gym.Env):
    ...
    def __init__(self, num_friendly_satellites, num_enemy_satellites,
                 ...
                 allow_action_interruption: bool = True):
        super().__init__()
        ...
        self.allow_action_interruption = allow_action_interruption # 存储开关状态
    ...
    def step(self, actions):
        ...
        for i in range(self.num_friendly):
            ...
            if agent_sat.status == 'destroyed': continue 

            action_dict = actions[f"agent_{i}"] # Get the new action for this agent
            new_action_type_from_agent = action_dict["action_type"]

            can_process_new_action = True # Flag to determine if the new action should be processed
            
            is_busy = agent_sat.action_duration_remaining > 1e-5 # Check if agent is currently busy

            if is_busy:
                if self.allow_action_interruption:
                    # If interruption is allowed and new action is NOT NO_OP, then interrupt.
                    if new_action_type_from_agent != 0: # 0 is NO_OP
                        # print(f"DEBUG: Agent {i} interrupting current action '{agent_sat.current_action_type}' with new action type {new_action_type_from_agent}")
                        # Clean up any temporary variables from the interrupted action
                        if agent_sat.current_action_type == "MANEUVER":
                            for attr in ['_maneuver_initial_pos', '_maneuver_initial_vel', '_maneuver_initial_orbit', '_maneuver_total_set_duration']:
                                if hasattr(agent_sat, attr):
                                    delattr(agent_sat, attr)
                        # Reset current action state to allow new action processing
                        agent_sat.current_action_type = None 
                        agent_sat.action_duration_remaining = 0.0
                        agent_sat.is_changing_formation = False # Reset if it was changing formation
                        # can_process_new_action remains True, new action will be processed below
                    else: 
                        # New action is NO_OP, agent is busy, interruption allowed.
                        # This means "continue current long action". So, we don't process this NO_OP.
                        can_process_new_action = False
                else: 
                    # Agent is busy, and interruption is NOT allowed.
                    can_process_new_action = False
            
            # If agent was not busy, or if an interruption occurred and a new significant action is given:
            if can_process_new_action:
                # Reset instantaneous task states before processing new action
                agent_sat.observing_target_id = None
                agent_sat.attacking_target_id = None
                if new_action_type_from_agent == 0: agent_sat.current_action_type = "NO_OP"
                elif new_action_type_from_agent == 1: 
                ...
                elif new_action_type_from_agent == 2: 
                ...
                elif new_action_type_from_agent == 3: 
                ...
                elif new_action_type_from_agent == 4: 
        ...
        for sat_idx, sat in enumerate(self.satellites):
            ...
            if sat.action_duration_remaining > 1e-5: 
                ...
                if sat.current_action_type == "MANEUVER":
                    ...
                    if total_maneuver_time > 1e-6:
                        elapsed_maneuver_time = total_maneuver_time - (sat.action_duration_remaining - time_consumed_this_step)
                        progress_fraction = min(elapsed_maneuver_time / total_maneuver_time, 1.0)
                    else: 
